...
describe("Host Connectors route", () => {
  ...
  it("returns a one-command local pairing flow", async () => {
    ...
    await expect(response.json()).resolves.toEqual({
      ...
      expiresAt: 10_000,
      command:
        "curl --proto '=https' --tlsv1.2 -fsSL https://overtchat.com/install/connector/0.12.0 | sh -s -- --server 'http://127.0.0.1:9000' --pair-code 'ocp_pair.secret'",
    });
      ...
      connectors: [
        {
          ...
          upgrade: {
            version: "0.12.0",
            command:
              "curl --proto '=https' --tlsv1.2 -fsSL https://overtchat.com/install/connector/0.12.0 | sh -s -- --upgrade",
          },
        },
      ],
    });
