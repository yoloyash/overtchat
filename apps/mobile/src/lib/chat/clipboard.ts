...
export async function readClipboardImage(): Promise<PickedFile | null> {
  ...
  return cacheClipboardImage(image.data);
}

export function cacheClipboardImage(data: string): PickedFile {
  const base64Start = data.indexOf(BASE64_MARKER);
  if (!data.startsWith("data:image/") || base64Start < 0) {
  ...
  file.create();
  file.write(data.slice(base64Start + BASE64_MARKER.length), {
